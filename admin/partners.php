...
if ($op == 'delete') {
    $id = $_GET['id'];
    $sql = "SELECT foto FROM partners WHERE id = '$id'";
...
?>
<h1 class="py-3">Partners Admin</h1>
<p>
    <a href="partners_input.php">
        <input type="button" value="Tambah partners" class="btn btn-primary">
    </a>
</p>
    ...
    <tbody>
        ...
        if ($katakunci != '') {
            ...
            for ($x = 0 ; $x < count($array_katakunci) ; $x++) { 
                $cari[] = "(nama LIKE '%".$array_katakunci[$x]."%' OR isi LIKE '%".$array_katakunci[$x]."%')";
            }
            $tambahan = " WHERE ".implode(" OR ", $cari);
        }
        $sql = "SELECT * FROM partners $tambahan";  //? Perintah untuk search
        ...
        ?>
            <tr>
                ...
                <td><img src="../img/<?= partnersPict($result['id']) ?>" style="max-height: 100px; max-width 100px"></td>
                <td><?= $result['nama'] ?></td>
                <td>
                    <a href="partners_input.php?id=<?= $result['id']?>">
                        <span class="badge bg-warning text-dark">Edit</span>
                    </a>
                    <a href="partners.php?op=delete&id=<?= $result['id']?>" onclick=" return confirm('Yakin ingin dihapus ?')">
...
<nav aria_label="Page navigation exaple">
    <ul class="pagination">
        ...
        ?>
            <li class="page-item">
                <a href="partners.php?katakunci=<?= $katakunci ?>&cari=<?= $cari ?>&page=<?= $i ?>" class="page-link">
                    <?= $i ?>
                </a>
            </li>
        <?php } ?>

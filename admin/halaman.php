...
if ($op == 'delete') {
    $id = $_GET['id'];
    $tampil = "DELETE FROM halaman WHERE id = '$id'";
...
?>
<h1 class="py-3">Halaman Admin</h1>
<p>
    <a href="hal_input.php">
        <input type="button" value="Tambah Halaman" class="btn btn-primary">
    </a>
</p>
    ...
    <tbody>
        ...
        if ($katakunci != '') {
            ...
            for ($x = 0 ; $x < count($array_katakunci) ; $x++) { 
                $cari[] = "(judul LIKE '%".$array_katakunci[$x]."%' OR kutipan LIKE '%".$array_katakunci[$x]."%' OR isi LIKE '%".$array_katakunci[$x]."%')";
            }
            $tambahan = " WHERE ".implode(" OR ", $cari);
        }
        $tampil = "SELECT * FROM halaman $tambahan";  //? Perintah untuk search
        ...
        ?>
            <tr>
                ...
                <td><?= $result['judul']?></td>
                <td><?= $result['kutipan'] ?></td>
                <td>
                    <a href="hal_input.php?id=<?= $result['id']?>">
                        <span class="badge bg-warning text-dark">Edit</span>
                    </a>
                    <a href="halaman.php?op=delete&id=<?= $result['id']?>" onclick=" return confirm('Yakin ingin dihapus ?')">
...
<nav aria_label="Page navigation exaple">
    <ul class="pagination">
        ...
        ?>
            <li class="page-item">
                <a href="halaman.php?katakunci=<?= $katakunci ?>&cari=<?= $cari ?>&page=<?= $i ?>" class="page-link">
                    <?= $i ?>
                </a>
            </li>
        <?php } ?>

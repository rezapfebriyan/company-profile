...
if ($op == 'delete') {
    $id = $_GET['id'];
    $sql = "SELECT foto FROM tutors WHERE id = '$id'";
...
?>
<h1 class="py-3">Tutors Admin</h1>
<p>
    <a href="tutors_input.php">
        <input type="button" value="Tambah Tutors" class="btn btn-primary">
    </a>
</p>
...
<table class="table table-striped mt-4">
    ...
    <tbody>
        ...
        $sql = "SELECT * FROM tutors $tambahan";  //? Perintah untuk search
        ...
        ?>
            <tr>
                ...
                <td><img src="../img/<?= tutorsPict($result['id']) ?>" style="max-height: 100px; max-width 100px"></td>
                <td><?= $result['nama'] ?></td>
                <td>
                    <a href="tutors_input.php?id=<?= $result['id']?>">
                        <span class="badge bg-warning text-dark">Edit</span>
                    </a>
                    <a href="tutors.php?op=delete&id=<?= $result['id']?>" onclick=" return confirm('Yakin ingin dihapus ?')">
...
<nav aria_label="Page navigation exaple">
    <ul class="pagination">
        ...
        ?>
            <li class="page-item">
                <a href="tutors.php?katakunci=<?= $katakunci ?>&cari=<?= $cari ?>&page=<?= $i ?>" class="page-link">
                    <?= $i ?>
                </a>
            </li>
        <?php } ?>
